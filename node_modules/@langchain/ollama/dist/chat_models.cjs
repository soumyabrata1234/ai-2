...
let ollama_browser = require("ollama/browser");
...
* <br />
*/
var ChatOllama = class extends _langchain_core_language_models_chat_models.BaseChatModel {
	static lc_name() {
		return "ChatOllama";
	}
	model = "llama3";
	...
	constructor(modelOrFields, fieldsArg) {
		...
		super(fields);
		this._addVersion("@langchain/ollama", "1.2.7");
		this.baseUrl = fields.baseUrl ?? (0, _langchain_core_utils_env.getEnvironmentVariable)("OLLAMA_BASE_URL") ?? this.baseUrl;
	...
	*/
	...
	async _generate(messages, options, runManager) {
		...
		else finalChunk = (0, _langchain_core_utils_stream.concat)(finalChunk, chunk.message);
		const nonChunkMessage = new _langchain_core_messages.AIMessage({
			id: finalChunk?.id,
	...
	withStructuredOutput(outputSchema, config) {
		...
		if (method === "functionCalling") {
			...
			llm = this.bindTools([{
				...
				kwargs: { method },
				schema: (0, _langchain_core_utils_types.isInteropZodSchema)(schema) || (0, _langchain_core_utils_standard_schema.isSerializableSchema)(schema) ? jsonSchema : schema
			} });
		...
		return (0, _langchain_core_language_models_structured_output.assembleStructuredOutputPipeline)(llm, outputParser, includeRaw, includeRaw ? "StructuredOutputRunnable" : "ChatOllamaStructuredOutput");
	}
};
//#endregion
